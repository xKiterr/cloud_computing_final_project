...
public class StorageOperations {
    ...
    public void makeBlobPublic() throws Exception {
        ...
        System.out.println("Blob '" + blobName + "' in bucket '" + bucketName + "' is now public.");
    }

    public void downloadFileAutomatically(String bucketName, String blobName) {
        try {
            Path downloadTo = Paths.get("downloaded_" + blobName);
            BlobId blobId = BlobId.of(bucketName, blobName);
            Blob blob = storage.get(blobId);

            if (blob == null) {
                System.err.println("Blob " + blobName + " not found!");
                return;
            }
            ...
            System.out.println("File saved to your computer as: " + downloadTo.toAbsolutePath());
        } catch (Exception e) {
